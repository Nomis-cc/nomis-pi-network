        ...
        ISingletonService
    {
            ...
            TokensDataRequest request)
        {
            ...
            foreach (var tokensProvider in validProviders)
            {
                ...
                if (request.Blockchain != Chain.None && (urls.ContainsKey(request.Blockchain) || urls.ContainsKey(Chain.None)))
                {
                    urls = urls
                        .Where(u => u.Key == request.Blockchain || u.Key == Chain.None)
                        .ToDictionary(u => u.Key, w => w.Value);
                }
            ...
            BlockchainSwapPairsRequest request)
        {
            ...
            var dexTokensData = await TokensDataAsync(new TokensDataRequest
            {
